...
define(['require','util','dependencies/EventEmitter'],function(require,util, EventEmitter){
	...
	// include stylesheet.
	util.registerStylesheet(require.toUrl('UI/Widget/TreeList.css'));
	...
	function TreeList(list,options){
		...
		list.forEach(function(itemObj,index){
			...
			// set zebra stripe classes if indicated.
			if ( options.zebraStripe )
			{
				if ( options.zebraStripe == 'even' && ! ( index % 2 ) )
				{
					itemClasses.push('zebra');
				}
				else if ( options.zebraStripe == 'odd' && (index % 2) )
				{
					itemClasses.push('zebra');
				}
