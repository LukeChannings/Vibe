 ...
 * my_inst.emit('someEvent', {eventProp:'value'});
 * 
 * @example
 * // create a 'class'
 * MyClass = function() {}
 ...
 * // create a method, which emits an event
 * MyClass.prototype.scrollComplete = function() {
 *     this.emit('scrolled', {baz:'eck'});
 * };
 * 
 ...
 * // fire the method, which should emit the event and therefore our callback
 ...
 * @param {function} callback the callback function to fire when the event is emited
 ...
 * the callback will be unbound after the event is first emited
 *
 * @param {string} name the name of the event
 * @param {function} callback the callback function to fire when the event is emited
 * @param {object} context to use for the callback (which will become 'this' inside the callback)
...
/**
 * emit an event, firing all bound callbacks
 * 
 * @param {string} name the name of the event
